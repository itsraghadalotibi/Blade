...
Tooltip(
  message: (profile.socialMediaLinks?['GitHub'] != null &&
      ...
      ? 'Go to GitHub'
      : 'No GitHub URL available', // Tooltip message based on URL presence
  child: IconButton(
    icon: FaIcon(
      FontAwesomeIcons.github,
      color: (profile.socialMediaLinks?['GitHub'] != null &&
              profile.socialMediaLinks!['GitHub']!.isNotEmpty)
          ? TColors.primary // Orange color for active GitHub URL
          : Colors.grey, // Gray color when no URL is available
      size: 24,
    ),
    padding: const EdgeInsets.all(0), // Remove default padding around the icon
    constraints: const BoxConstraints(), // Keep the icon button small
    onPressed: (profile.socialMediaLinks?['GitHub'] != null &&
                profile.socialMediaLinks!['GitHub']!.isNotEmpty)
        ? () {
            // Launch GitHub URL if available
...
const SizedBox(width: 4), // Adjust this as needed for spacing

// LinkedIn Icon with Tooltip
Tooltip(
  message: (profile.socialMediaLinks?['LinkedIn'] != null &&
            profile.socialMediaLinks!['LinkedIn']!.isNotEmpty)
      ? 'Go to LinkedIn'
      : 'No LinkedIn URL available', // Tooltip message based on URL presence
  child: IconButton(
    icon: FaIcon(
      FontAwesomeIcons.linkedin,
      color: (profile.socialMediaLinks?['LinkedIn'] != null &&
              profile.socialMediaLinks!['LinkedIn']!.isNotEmpty)
          ? TColors.primary // Orange color for active LinkedIn URL
          : Colors.grey, // Gray color when no URL is available
      size: 24,
    ),
    padding: const EdgeInsets.all(0), // Remove default padding around the icon
    ...
    onPressed: (profile.socialMediaLinks?['LinkedIn'] != null &&
                profile.socialMediaLinks!['LinkedIn']!.isNotEmpty)
        ? () {
            // Launch LinkedIn URL if available
