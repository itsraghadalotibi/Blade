...
class AvatarStackWidget extends StatelessWidget {
  ...
  AvatarStackWidget({required this.userIds, required this.repository, required this.screenWidth});

  @override
  Widget build(BuildContext context) {
    final avatarSize = screenWidth * 0.07; // Fixed size for all avatars
    final maxVisibleAvatars = 3; // Show a maximum of 3 avatars
    final hasExtraAvatars = userIds.length > maxVisibleAvatars;

    // Fixed width for the avatar stack based on the number of avatars
    final stackWidth = (hasExtraAvatars ? (maxVisibleAvatars + 1) : userIds.length) * avatarSize * 0.6;
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          ...
          for (int i = 0; i < (hasExtraAvatars ? maxVisibleAvatars : userIds.length); i++)
            Positioned(
              left: i * (avatarSize * 0.6), // Offset each avatar by 60% of its size
          ...
          if (hasExtraAvatars)
            Positioned(
              left: maxVisibleAvatars * (avatarSize * 0.6), // Place extra avatar at the end
              ...
              child: SizedBox(
                ...
                child: CircleAvatar(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(color: Color(0xFFFD5336)),
                  ),
                ),
              ),
            ),
